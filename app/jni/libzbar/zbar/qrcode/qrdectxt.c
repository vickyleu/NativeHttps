...
int qr_code_data_list_extract_text(const qr_code_data_list *_qrlist,
                                   zbar_image_scanner_t *iscn, zbar_image_t *img) {
    iconv_t sjis_cd;
    iconv_t utf8_cd;
    iconv_t latin1_cd;
        ...
        if (!mark[i]) {
                    ...
                    switch (entry->mode) {
                        ...
                        case QR_MODE_KANJI: {
                            ...
                            if (eci < 0) {
                                ...
                                else if (inleft >= 3 && in[0] == (char) 0xEF
                                         && in[1] == (char) 0xBB
                                         && in[2] == (char) 0xBF) {
                                    in += 3;
                                    inleft -= 3;
                                ...
                                for (ei = 0; ei < 3; ei++)
                                    if (enc_list[ei] != (iconv_t) -1) {
                                        ...
                                        err = iconv(enc_list[ei], &in, &inleft,
                                                    &out, &outleft) == (size_t) -1;
                                        if (!err) {
                                            sa_ntext = out - sa_text;
            ...
            if (!err) {
                ...
                if (sa_ctext + 1 > sa_ntext) {
                    sa_text = (char *) realloc(sa_text,
                                               sa_ntext * sizeof(*sa_text));
                }
                ...
                if (sa_size == 1)
                    ...
                    sa_sym = _zbar_image_scanner_alloc_sym(iscn, ZBAR_QRCODE,
                                                           0);
